...
!*******************************************************************************
  ...
  TE = truncation_error(xnew,ynew,alpha,nu,Lref,imax,jmax)
  Jfunc = calc_J(xnew,ynew,nu,alpha,Lref,imax,jmax)
  djdxmat = calc_djdx(xnew,ynew,nu,alpha,Lref,imax,jmax)
  djdx = djdxmat(:,:,1)
  djdy = djdxmat(:,:,2)
  u = uxn(xnew,ynew,alpha,nu,Lref,0)


  !write grid history
  write(fid_gridinfo,'(A)') 'zone T="Adapted"'
  write(fid_gridinfo,*) 'i=',imax
  write(fid_gridinfo,*) 'j=',jmax
  
  write(fid_gridinfo,'(7e23.15)') ((xnew(i,j),ynew(i,j),u(i,j),&
                                    TE(i,j),Jfunc,djdx(i,j),djdy(i,j),&
                                    i=1,imax),j=1,jmax)
