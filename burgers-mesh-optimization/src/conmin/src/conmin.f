...
c
     ...
     2RM,ICNDIR,IGOTO,NAC,INFO,INFOG,ITER,IPUNIT,IFUNIT,I1160,I1170,
     3I1180,I1190
C
...
C     RE-SCALE VARIABLES IF REQUIRED. 
      WRITE(7,*)'IN CONMIN IPUNIT = ', IPUNIT
      WRITE(7,*)'IN CONMIN CNMN1'
      ...
      WRITE(7,*)IOBJ,KOBJ,KCOUNT,NCAL(1),NCAL(2),NFEAS,MSCAL
      WRITE(7,*)NCOBJ,NVC,KOUNT,ICOUNT,IGOOD1,IGOOD2,IGOOD3
      WRITE(7,*)IGOOD4,IBEST,III,NLNC,JGOTO,ISPACE(1),ISPACE(2)
C
...
31         FORMAT(I8)
       ...
       WRITE(7,*)N1, N2, N3, N4, N5
C
C     WRITE(6,*)'IN CONMIN IPUNIT = ', IPUNIT
C     WRITE(6,*)'IN CONMIN OBJ, IPRINT = ', OBJ, IPRINT
C     WRITE(IPUNIT,*)'>>>>>RMK inCONMIN, IGOTO, NSCAL',IGOTO,NSCAL
...
C     ------------------------------------------------------------------
40    CONTINUE
      IF (IPRINT.GT.0) WRITE (IPUNIT,1220) 
CRMK>>>>>
C     DO 51 I=1,NDV
C      WRITE(IPUNIT,*)X(I)
C51    CONTINUE
C      WRITE (IPUNIT,*)LINOBJ, NCON, NSIDE
CRMK<<<<
...
C     SOLUTION IS UNBOUNDED.
      WRITE (IPUNIT,970) LINOBJ,NCON,NSIDE 
      RETURN
50    CONTINUE
      IDM1=ITRM 
      IDM2=ITMAX
...
C     LOWER BOUND VIOLATED. 
      WRITE (IPUNIT,1130) X(I),VLB(I),I
      X(I)=VLB(I) 
      GO TO 100 
90    CONTINUE
      XX=VUB(I)-X(I)
      IF (XX.GE.0.) GO TO 100 
      WRITE (IPUNIT,1140) X(I),VUB(I),I
      X(I)=VUB(I) 
100   CONTINUE
110   CONTINUE
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
      ...
      IF (NSIDE.EQ.0.AND.NCON.EQ.0) WRITE (IPUNIT,1290)
      IF (NSIDE.NE.0.OR.NCON.GT.0) WRITE (IPUNIT,1230) 
C2345678901234567890123456789012345678901234567890123456789012345678901234567890
C       10        20        30        40        50        60        70 2
      WRITE (IPUNIT,1240) IPRINT,NDV,ITMAX,NCON,NSIDE,ICNDIR,NSCAL,NFDG,
     1LINOBJ,ITRM,N1,N2,N3,N4,N5 
      WRITE (IPUNIT,1260) CT,CTMIN,CTL,CTLMIN,THETA,PHI,DELFUN,DABFUN
      WRITE (IPUNIT,1250) FDCH,FDCHM,ALPHAX,ABOBJ1 
      IF (NSIDE.EQ.0) GO TO 190 
      WRITE (IPUNIT,1270)
      DO 170 I=1,NDV,6
      M1=MIN0(NDV,I+5)
170   WRITE (IPUNIT,1010) I,(VLB(J),J=I,M1)
      WRITE (IPUNIT,1280)
      DO 180 I=1,NDV,6
      M1=MIN0(NDV,I+5)
180   WRITE (IPUNIT,1010) I,(VUB(J),J=I,M1)
190   CONTINUE
      IF (NSCAL.GE.0) GO TO 200 
      WRITE (IPUNIT,1300)
      WRITE (IPUNIT,1460) (SCAL(I),I=1,NDV)
200   CONTINUE
      ...
      DO 210 I=1,NCON,15
      M1=MIN0(NCON,I+14)
210   WRITE (IPUNIT,1030) I,(ISC(J),J=I,M1)
      GO TO 230 
220   IF (NLNC.EQ.NCON) WRITE (IPUNIT,1040)
      IF (NLNC.EQ.0) WRITE (IPUNIT,1050) 
230   CONTINUE
      WRITE (IPUNIT,1440) OBJ
      WRITE (IPUNIT,1450)
      DO 240 I=1,NDV
      X1=1. 
...
240   G1(I)=X(I)*X1 
      DO 250 I=1,NDV,6
      M1=MIN0(NDV,I+5)
250   WRITE (IPUNIT,1010) I,(G1(J),J=I,M1) 
      IF (NCON.EQ.0) GO TO 270
      WRITE (IPUNIT,1470)
      DO 260 I=1,NCON,6 
      M1=MIN0(NCON,I+5) 
260   WRITE (IPUNIT,1010) I,(G(J),J=I,M1)
270   CONTINUE
      IF (IPRINT.GT.1) WRITE (IPUNIT,1360) 
C     ------------------------------------------------------------------
...
C     IF (NFEAS.GT.10) GO TO 810
      IF (IPRINT.GT.2) WRITE (IPUNIT,1310) ITER
      IF (IPRINT.GT.3.AND.NCON.GT.0) WRITE (IPUNIT,1320) CT,CTL,PHI
      CTA=ABS(CT) 
C     WRITE(IPUNIT,*)'>>>RMK CAlling cnmn01 NCOBJ',NCOBJ
...
330   CONTINUE
      GO TO 420 
340   CONTINUE
C     WRITE(IPUNIT,*)'>>>RMK MSCAL, NSCAL, KCOUNT, ICNDIR'
C     WRITE(IPUNIT,*)MSCAL, NSCAL, KCOUNT, ICNDIR
...
C     ------------------------------------------------------------------
      INFO=2
      NCAL(2)=NCAL(2)+1 
C     WRITE(IPUNIT,*)'>>>RMK NFDG = ',NFDG
      IF (NFDG.NE.1) GO TO 370
C     WRITE(IPUNIT,*)'>>>RMK GOING to 950 NFDG = ',NFDG
      IGOTO=2 
      GO TO 950 
370   CONTINUE
      JGOTO=0 
380   CONTINUE
C     WRITE(IPUNIT,*)'>>>RMK AFTER 380 NFDG  '
...
390   CONTINUE
      INFO=1
C     WRITE(IPUNIT,*)'>>>RMK AFTER 390 NFDG NAC, N3', NAC, N3
      IF (NAC.GE.N3) GO TO 810
C     WRITE(IPUNIT,*)'>>>RMK AFTER 390 NSCAL NFDG ', NSCAL,NFDG
...
440   CONTINUE
450   M3=M2-N3
      WRITE (IPUNIT,1060) M1 
      IF (M1.EQ.0) GO TO 460
      WRITE (IPUNIT,1070)
      WRITE (IPUNIT,1480) (MS1(I),I=1,M1)
460   WRITE (IPUNIT,1080) M3 
      IF (M3.EQ.0) GO TO 470
      WRITE (IPUNIT,1070)
      M3=N3+1 
      WRITE (IPUNIT,1480) (MS1(I),I=M3,M2) 
470   CONTINUE
C     ------------------------------------------------------------------
...
C     PRINT ACTIVE SIDE CONSTRAINT NUMBERS. 
      ...
      WRITE (IPUNIT,1090) M1 
      IF (M1.EQ.0) GO TO 530
      WRITE (IPUNIT,1100)
      WRITE(IPUNIT,1480) (MS1(J),J=1,M1) 
530   CONTINUE
C     PRINT GRADIENTS OF ACTIVE AND VIOLATED CONSTRAINTS. 
      ...
      DO 540 I=1,NDV,6
      M1=MIN0(NDV,I+5)
540   WRITE (IPUNIT,1010) I,(DF(J),J=I,M1) 
      ...
      IF (M2.LE.0) WRITE (IPUNIT,990) M1 
      IF (M2.GT.0) WRITE (IPUNIT,1000) M3
      DO 550 K=1,NDV,6
      M1=MIN0(NDV,K+5)
550   WRITE (IPUNIT,1010) K,(A(J,I),J=K,M1)
560   WRITE (IPUNIT,1360)
570   CONTINUE
C     ------------------------------------------------------------------
...
C     IF (NVC.EQ.0) NFEAS=0 
      ...
      DO 590 I=1,NAC,6
      M1=MIN0(NAC,I+5)
590   WRITE (IPUNIT,1010) I,(A(NDV1,J),J=I,M1) 
      WRITE (IPUNIT,1210) S(NDV1)
600   CONTINUE
C     ------------------------------------------------------------------
...
640   CONTINUE
      ...
      DO 650 I=1,NDV,6
      M1=MIN0(NDV,I+5)
650   WRITE (IPUNIT,1010) I,(S(J),J=I,M1)
      WRITE (IPUNIT,1110) SLOPE,ALP
660   CONTINUE
...
C     PRINT MOVE PARAMETER, NEW X-VECTOR AND CONSTRAINTS. 
      ...
      WRITE (IPUNIT,1390) ALP
730   IF (IPRINT.LT.2) GO TO 800
      IF (OBJB.GT.0.) GO TO 740 
      IF (IPRINT.EQ.2) WRITE (IPUNIT,1400) ITER,OBJ
      IF (IPRINT.GT.2) WRITE (IPUNIT,1410) OBJ 
      GO TO 760 
740   IF (IPRINT.EQ.2) GO TO 750
      WRITE (IPUNIT,1420) OBJ
      GO TO 760 
750   WRITE (IPUNIT,1430) ITER,OBJ 
760   WRITE (IPUNIT,1450)
      DO 770 I=1,NDV
      FF1=1.
...
770   G1(I)=FF1*X(I)
      DO 780 I=1,NDV,6
      M1=MIN0(NDV,I+5)
780   WRITE (IPUNIT,1010) I,(G1(J),J=I,M1) 
      IF (NCON.EQ.0) GO TO 800
      WRITE (IPUNIT,1470)
      DO 790 I=1,NCON,6 
      M1=MIN0(NCON,I+5) 
790   WRITE (IPUNIT,1010) I,(G(J),J=I,M1)
800   CONTINUE
C 
...
C     ------------------------------------------------------------------
      ...
      GO TO 280 
810   CONTINUE
      IF (NAC.GE.N3) WRITE (IPUNIT,1490) 
C     ------------------------------------------------------------------
...
830   IF (IPRINT.EQ.0.OR.NAC.GE.N3) GO TO 940 
      ...
      DO 840 I=1,NDV,6
      M1=MIN0(NDV,I+5)
840   WRITE (IPUNIT,1010) I,(X(J),J=I,M1)
      IF (NCON.EQ.0) GO TO 900
      WRITE (IPUNIT,1470)
      DO 850 I=1,NCON,6 
      M1=MIN0(NCON,I+5) 
850   WRITE (IPUNIT,1010) I,(G(J),J=I,M1)
...
860   NVC=NVC+1 
      MS1(NVC)=I
870   CONTINUE
      WRITE (IPUNIT,1060) NAC
      IF (NAC.EQ.0) GO TO 880 
      WRITE (IPUNIT,1070)
      WRITE (IPUNIT,1480) (IC(J),J=1,NAC)
880   WRITE (IPUNIT,1080) NVC
      IF (NVC.EQ.0) GO TO 890 
      WRITE (IPUNIT,1070)
      WRITE (IPUNIT,1480) (MS1(J),J=1,NVC) 
890   CONTINUE
900   CONTINUE
...
910   XID=VUB(I)
      ...
      MS1(NAC)=I
920   CONTINUE
      WRITE (IPUNIT,1090) NAC
      IF (NAC.EQ.0) GO TO 930 
      WRITE (IPUNIT,1100)
      WRITE (IPUNIT,1480) (MS1(J),J=1,NAC) 
930   CONTINUE
      WRITE (IPUNIT,1150)
CRMK ADD I1160-I1190 to get convergence criteria out
      I1160 = 0
      I1170 = 0
      I1180 = 0
      I1190 = 0
      ...
      IF (IOBJ.GE.ITRM) THEN
          WRITE (IPUNIT,1180) ITRM 
          I1180 = 1
      END IF
      IF (KOBJ.GE.ITRM) THEN
         WRITE (IPUNIT,1190) ITRM 
         I1190 = 1 
      END IF
      WRITE (IPUNIT,1200) ITER 
      WRITE (IPUNIT,1510) NCAL(1)
      IF (NCON.GT.0) WRITE (IPUNIT,1520) NCAL(1) 
      IF (NFDG.NE.0) WRITE (IPUNIT,1530) NCAL(2) 
      IF (NCON.GT.0.AND.NFDG.EQ.1) WRITE (IPUNIT,1540) NCAL(2) 
C     ------------------------------------------------------------------
...
940   ITRM=IDM1 
      ...
      IGOTO=0 
950   CONTINUE
C     WRITE(IPUNIT,*)'>>>>RMK AT 950, NSCAL', NSCAL
      IF (NSCAL.EQ.0.OR.IGOTO.EQ.0) RETURN
C     UN-SCALE VARIABLES. 
      DO 960 I=1,NDV
      C(I)=X(I) 
960   X(I)=X(I)*SCAL(I) 
C     WRITE(IPUNIT,*)'>>>>RMK returning from 950'
      RETURN
C     ------------------------------------------------------------------
C                                FORMATS
C     ------------------------------------------------------------------
...
10    IF (SLOPE.LT.0.) GO TO 20 
      ...
      RETURN
20    CONTINUE
      IF (IPRINT.GT.4) WRITE (IPUNIT,360)
      FFF=OBJ 
      AP1=0.
      A1=0. 
      F1=OBJ
...
30    CONTINUE
      ...
      DO 40 I=1,NDV 
40    X(I)=X(I)+AP*S(I) 
      IF (IPRINT.GT.4) WRITE (IPUNIT,370) AP 
      IF (IPRINT.GT.4) WRITE (IPUNIT,380) (X(I),I=1,NDV) 
      NCAL(1)=NCAL(1)+1 
      JGOTO=1 
      RETURN
50    CONTINUE
      F2=OBJ
      IF (IPRINT.GT.4) WRITE (IPUNIT,390) F2 
...
C     ------------------------------------------------------------------
      ...
      DO 70 I=1,NDV 
70    X(I)=X(I)+AP*S(I) 
      IF (IPRINT.GT.4) WRITE (IPUNIT,370) A2 
      IF (IPRINT.GT.4) WRITE (IPUNIT,380) (X(I),I=1,NDV) 
      NCAL(1)=NCAL(1)+1 
      JGOTO=2 
      RETURN
...
C     ------------------------------------------------------------------
      ...
      DO 130 I=1,NDV
130   X(I)=X(I)+AP*S(I) 
      IF (IPRINT.GT.4) WRITE (IPUNIT,370) A3 
      IF (IPRINT.GT.4) WRITE (IPUNIT,380) (X(I),I=1,NDV) 
      NCAL(1)=NCAL(1)+1 
      JGOTO=4 
      RETURN
140   CONTINUE
      F3=OBJ
      IF (IPRINT.GT.4) WRITE (IPUNIT,390) F3 
150   CONTINUE
...
C     ------------------------------------------------------------------
      ...
      DO 170 I=1,NDV
170   X(I)=X(I)+AP*S(I) 
      IF (IPRINT.GT.4) WRITE (IPUNIT,370) ALP
      IF (IPRINT.GT.4) WRITE (IPUNIT,380) (X(I),I=1,NDV) 
      NCAL(1)=NCAL(1)+1 
      JGOTO=5 
      RETURN
180   CONTINUE
      IF (IPRINT.GT.4) WRITE (IPUNIT,390) OBJ
C     ------------------------------------------------------------------
C                         CHECK CONVERGENCE 
C     ------------------------------------------------------------------
      AA=1.-APP/A2
...
220   CONTINUE
      F4=OBJ
      IF (IPRINT.GT.4) WRITE (IPUNIT,390) F4 
...
C     ZERO GRADIENT IS FOUND.  WRITE ERROR MESSAGE. 
      IF(IPRINT.GE.2) WRITE(IPUNIT,43)IC(I)
...
C     ------------------------------------------------------------------
      ...
      IF (IPRINT.GT.1.AND.NER.GT.0) WRITE (IPUNIT,180) 
...
C     S-VECTOR IS NOT FEASIBLE DUE TO SOME NUMERICAL PROBLEM. 
      IF(IPRINT.GE.2) WRITE(IPUNIT,178)
...
10    IF (IPRINT.GE.5) WRITE (IPUNIT,730)
      ALPSAV=ALP
      ICOUNT=0
      ALPTOT=0. 
C     TOLERANCES. 
...
100   CONTINUE
      ...
      DO 110 I=1,NDV
110   G(I)=SCAL(I)*X(I) 
      WRITE (IPUNIT,750) (G(I),I=1,NDV)
      GO TO 130 
120   WRITE (IPUNIT,750) (X(I),I=1,NDV)
C     ------------------------------------------------------------------
...
140   CONTINUE
      F2=OBJ
      IF (IPRINT.GE.5) WRITE (IPUNIT,760) F2 
      IF (IPRINT.LT.5.OR.NCON.EQ.0) GO TO 150 
      WRITE (IPUNIT,770) 
      WRITE (IPUNIT,750) (G(I),I=1,NCON) 
150   CONTINUE
C     ------------------------------------------------------------------
...
280   G(I)=SCAL(I)*X(I) 
      WRITE (IPUNIT,750) (G(I),I=1,NDV)
      GO TO 300 
290   WRITE (IPUNIT,750) (X(I),I=1,NDV)
300   CONTINUE
C     ------------------------------------------------------------------
C              UPDATE FUNCTION AND CONSTRAINT VALUES
C     ------------------------------------------------------------------
...
310   CONTINUE
      F3=OBJ
      IF (IPRINT.GE.5) WRITE (IPUNIT,760) F3 
...
490   G(I)=SCAL(I)*X(I) 
      WRITE (IPUNIT,750) (G(I),I=1,NDV)
      GO TO 510 
500   WRITE (IPUNIT,750) (X(I),I=1,NDV)
510   CONTINUE
C     ------------------------------------------------------------------
C              UPDATE FUNCTION AND CONSTRAINT VALUES
C     ------------------------------------------------------------------
...
520   CONTINUE
      F4=OBJ
      IF (IPRINT.GE.5) WRITE (IPUNIT,760) F4 
...
710   CONTINUE
      ALP=ALPTOT
      IF (IPRINT.GE.5) WRITE (IPUNIT,790)
      JGOTO=0 
      RETURN
C     ------------------------------------------------------------------
C                                  FORMATS
