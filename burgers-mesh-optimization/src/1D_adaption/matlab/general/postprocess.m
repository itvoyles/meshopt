...
for ii = III(1:end),
    ...
    copyfile( './postplot.m',strcat('./Case33',num2str(ii)) )
    copyfile( './postplot.m',strcat('./Case65',num2str(ii)) )
%     copyfile( './postplot.m',strcat('./Case129',num2str(ii)) )
...
for ii = III(1:end),
    ...
    run( strcat('./Case33',num2str(ii),'/postplot.m') )
    
    figure(1)
    set(gcf,'NextPlot','add');
    axes;
    h = title( char( ...
        strcat('imax = 33; Flux: ',char(var(ii,4)),'; Limiter: ',char(var(ii,2)), ...
        '; Functional: ',char(var(ii,3)),'; Solution: ',char(var(ii,1))), ... First line
        'Func        Cont        X-Mtm       Energy', ... Second line
        strcat(num2str(fin(2,3)/fin(1,3),'%7.2e'),'---',num2str(fin(2,4),'%7.2f'), ...
        '---',num2str(fin(2,5),'%7.2f'),'---',num2str(fin(2,6),'%7.2f')) ) ) ; % Third line
    set(gca,'Visible','off');
    set(h,'Visible','on');
    
    print -append -dpsc2 outplots.ps
    
%     %%%%%%%% imax = 65
    
    run( strcat('./Case65',num2str(ii),'/postplot.m') )
    figure(1)
    set(gcf,'NextPlot','add');
    axes;
    h = title( char( ...
        strcat('imax = 65; Flux: ',char(var(ii,4)),'; Limiter: ',char(var(ii,2)), ...
        '; Functional: ',char(var(ii,3)),'; Solution: ',char(var(ii,1))), ... First line
        'Func        Cont        X-Mtm       Energy', ... Second line
        strcat(num2str(fin(2,3)/fin(1,3),'%7.2e'),'---',num2str(fin(2,4),'%7.2f'), ...
        '---',num2str(fin(2,5),'%7.2f'),'---',num2str(fin(2,6),'%7.2f')) ) ) ; % Third line
...
%     %%%%%%%% imax = 129
%     
%     run( strcat('./Case129',num2str(ii),'/postplot.m') )
%     figure(1)
%     set(gcf,'NextPlot','add');
%     axes;
%      h = title( char( ...
