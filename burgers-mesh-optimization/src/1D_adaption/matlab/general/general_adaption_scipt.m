...
global k xthroat toler astar tzero clls imax eqn cell_vol cells_hold param
...
if numel(imax) ~= 0,
    ...
    imax = 33; % Number of grid nodes/ FVM faces (choose odd)
end

cells_hold = 1;                    % Cells from the inlet that are held in place (=1 is normal)
eqn = 'q1d';                       % Case used:
...
elseif strcmp(eqn,'q1d'),
    ...
    if numel(sol) ~= 0,
        ...
        sol = 'super'; % Specify exact solution type: 'sub' -- Isentropic Subsonic-subsonic
    ...
    lim_type = 'vanleer'; % Available: 'ospre','minmod','superbee','sweby','vanalbada','vanleer','off'              
    end
    ...
    if numel(functional) ~= 0,
        ...
        functional = 'rss'; % Type of functional used: 'cont'  : continuity
                             %                          'xmtm'  : x-momentum
                             %                          'energy': energy
                             %                          'rss'   : root sum square
    end
    ...
    param = imax; % For uniform calculations; for cells_hold>1, adjusts later
    tzero = 300.0; % K, Stagnation temperature
    pzero = 300000.0; % Pa, Chamber pressure
    order = 7; % Order of the quadrature
    toler = 1E-12; % Iterative tolerance (1E-12), used for exact solutions
    ...
    cell_vol = zeros(imax,3); % Cell volume, used to scale TE
...
readgrid = 2;           % read grid file 'grid.grd' = 1, create grid = 0
                        % create non-uniform grid = 2
...
%     cell_vol = [cell_vol, cell_vol, cell_vol]; % For appropriate matrix size
    [TE] = q1d_TE( imax,xeq,pb,pzero,sol,order,kappa,flag,flux_type,lim_type,shloc,As2 );
    
    % Scale by cell volume: Cell-center area by dx
%     TE = TE./cell_vol;
...
param = imax - cells_hold + 1; % Adjust problem size for cells_hold>1
leftbndry = xeq(cells_hold);
...
elseif strcmp(method,'spring-f')
    springsystem_force_setup(param);
    [A,b,Aeq,beq] = spring_f_linear_constraints(param,eps_small,leftbndry,rightbndry);
...
% -------------------------------------------------------------------------
x = x0;
DV = reverse_mapping(x0(cells_hold:end));
% -------------------------------------------------------------------------
% Options
% -------------------------------------------------------------------------
...
x2 = forward_mapping(DV);
x(cells_hold:imax) = x2;
fprintf('Done\n');

param = imax; % Reset "param" to "imax" for final calculations
%% Post processing
...
%% spring-f functions
    ...
    global SSAinv k
        ...
        Aeq(1,1) = 1;
        Aeq(2,end)=1;
%         Aeq(3,k) = 1; % Uncomment to pin a node (face) at the throat

        beq = zeros(3,1);
        beq(1) = leftbndry;
        beq(2) = rightbndry;
%         beq(3) = 0; % Uncomment to pin a node (face) at the throat

        % The code for pinning a node at the throat assumes that a uniform
        % grid is being given at that outset, or else the index of the node
        % is the same as the uniform grid case (e.g., a grid with spacing
        % symmetric about the throat).
